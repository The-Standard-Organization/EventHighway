...
namespace EventHighway.Core.Services.Coordinations.Events.V2
{
    internal partial class EventV2CoordinationService
    {
        ...
        private delegate ValueTask<IReadOnlyList<EventV2>> ReturningEventV2ListFunction();
        ...
        private async ValueTask<IReadOnlyList<EventV2>> TryCatch(
            ReturningEventV2ListFunction returningEventV2ListFunction)
        {
            try
            {
                return await returningEventV2ListFunction();
            }
            catch (OperationCanceledException operationCanceledException)
                when (operationCanceledException.CancellationToken.IsCancellationRequested is false)
            {
